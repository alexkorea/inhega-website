    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-10 shadow-sm text-center">
        ...
        <Link
          ...
          className="inline-flex items-center justify-center bg-[#f36c24] hover:bg-[#d14904] text-white px-8 h-12 rounded-lg font-semibold transition-colors text-lg mb-2"
        >
          상세정보 입력하기 →
        </Link>
        <p className="text-sm text-slate-400 mb-6">약 1분 소요</p>

        <div className="bg-[#ffe6d5]/50 border border-[#f36c24]/20 rounded-xl p-6 mb-6 text-left">
          <h3 className="font-bold text-[#7c2d12] text-lg mb-3">비전행정사사무소</h3>
          <ul className="space-y-2 text-sm text-[#9a3412]">
            <li className="flex items-start gap-2"><span className="text-[#f36c24] mt-0.5">✓</span> 8년+ 인허가 전문 실무 경험</li>
            <li className="flex items-start gap-2"><span className="text-[#f36c24] mt-0.5">✓</span> 업종별 맞춤 인허가 컨설팅</li>
            <li className="flex items-start gap-2"><span className="text-[#f36c24] mt-0.5">✓</span> 서류 준비부터 접수·수령까지 원스톱 대행</li>
            <li className="flex items-start gap-2"><span className="text-[#f36c24] mt-0.5">✓</span> 전국 어디서나 온라인 상담 가능</li>
          </ul>
        </div>
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input name="name" type="text" required className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-[#f36c24] focus:outline-none focus:ring-1 focus:ring-[#f36c24]" placeholder="홍길동" />
        </div>

        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">이메일 <span className="text-red-500">*</span></label>
          <input name="email" type="email" required className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-[#f36c24] focus:outline-none focus:ring-1 focus:ring-[#f36c24]" placeholder="example@email.com" />
        ...
        <div>
          ...
          <input name="contact" type="text" className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-[#f36c24] focus:outline-none focus:ring-1 focus:ring-[#f36c24]" placeholder="010-1234-5678" />
        </div>

        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">SNS ID</label>
          <div className="flex gap-2">
            <select name="snsType" className="rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 focus:border-[#f36c24] focus:outline-none focus:ring-1 focus:ring-[#f36c24]">
            ...
            <input name="snsId" type="text" className="flex-1 rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-[#f36c24] focus:outline-none focus:ring-1 focus:ring-[#f36c24]" placeholder="SNS ID 입력" />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {services.map((svc) => (
              <button
                ...
                className={`relative flex items-center gap-3 p-3.5 rounded-xl border-2 text-left transition-all ${
                  selectedServices.includes(svc.value)
                    ? "border-[#f36c24] bg-[#ffe6d5] shadow-sm"
              ...
              >
                ...
                {selectedServices.includes(svc.value) && (
                  <div className="absolute top-2 right-2 w-5 h-5 bg-[#f36c24] rounded-full flex items-center justify-center">
        ...
        <button
          ...
          className="w-full rounded-lg bg-[#f36c24] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#d14904] disabled:bg-slate-300 disabled:cursor-not-allowed"
        >
