    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="inline-flex items-center px-4 py-1.5 rounded-full bg-[#ffe6d5] text-[#f36c24] text-sm font-medium mb-4">
            전문가 팀
          </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 mb-12">
          {admins.map((member) => (
            <div key={member.name} className="text-center group">
              <div className="w-24 h-24 mx-auto mb-3 rounded-full overflow-hidden border-2 border-slate-200 group-hover:border-[#f36c24] transition-colors">
                <Image
                  src={member.photo}
                  alt={member.name}
                  width={96}
